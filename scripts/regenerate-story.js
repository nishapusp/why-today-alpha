...
2026-08-10: apply the same "don't trust a source's phrasing blindly" discipline to anyone's ROLE, TITLE, or INCUMBENCY status — found necessary after a real, confirmed error: a story described a sitting head of state as a "former President," copied faithfully from an older article that was accurate when IT was written, before an election changed who held the office. Having search available does NOT automatically prevent this — search can surface a correctly-cited, on-topic source that is simply older than the story you're writing today. Before writing "current," "former," "outgoing," "newly appointed," or similar next to a named minister, CEO, chairperson, governor, or head of state, run a fresh, date-qualified search to confirm that status as of TODAY — do not infer it from how an older cited article phrased it. When unsure, use the role/title alone without the qualifier rather than asserting a status you can't confirm.
