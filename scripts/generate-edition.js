...
- If two sources give the central statistic two different numbers (even close ones, like 1.73% vs 1.8%), that is not noise to average away or pick from arbitrarily — it usually means they are measuring different things (different date, different segment, different methodology). Find the primary source and use its number, and use its exact framing of what it measures.
- This does not require re-verifying every minor figure in a story to this standard — it specifically applies to the one or two numbers a reader would remember as "the point" of the story.
...
2026-07-29: a real, confirmed error slipped past the rule above, and it's important to understand exactly how, because "3+ real numbers" as a series-level gut check is not sufficient — the failure was per-point, not series-level. A quarterly-trend chart had its two most recent points (the current quarter and the year-ago comparator) exactly correct, because both came straight from the same grounded results article. But its older historical quarters were invented — plausible-looking numbers that were not just wrong in magnitude but wrong in DIRECTION, showing a decline where the real trend was a steady rise. The two correct points made the whole chart look trustworthy; the fabricated points built a false "sharp recovery" narrative into the story's own takeaway text. This is a materially worse failure than an all-fabricated chart, because a reader (or a later spot-check) has real reason to trust the points they happen to check.
- Treat EVERY individual data point as its own claim requiring its own source — not "does this series look plausible/directionally right," but "did I actually find this specific quarter/period's number in a source." A results article for the current quarter typically only gives you that quarter plus its year-ago comparator; it does NOT give you the quarters in between or further back. Run a SEPARATE, dedicated search for each older period specifically (e.g. "[Company] Q2 FY25 net profit", "[Company] Q3 FY25 results") — the same discipline timeMachine's pastEvents already requires, applied here too.
- If some periods in a would-be series are genuinely sourced and others are not, do not fill the gaps with an estimate, an interpolation, or a "reasonable" number that fits the trend — DROP the unsourced periods from labels/values (keeping only the verified ones, still in chronological order) rather than inventing a value to complete a tidy-looking series. If fewer than 3 verified points remain after dropping, omit "chart" entirely for this story.
- A number that "sounds right" or fits an expected trend direction, recalled from general knowledge rather than found in this run's actual search results, is exactly what produced the false "decline" in the confirmed error above — recalled-from-training and verified-via-search feel identical from the inside, which is precisely why this failure mode is dangerous. If you cannot point to which specific source stated a specific historical value, that value does not go in the chart.
