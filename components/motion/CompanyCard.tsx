...
export default function CompanyCard({ headline, category, sentiment, stats, theme }: CompanyCardProps) {
  ...
  return (
    <div className="rounded-2xl p-5" style={{ background: theme.surface, border: `1px solid ${theme.accent}33` }}>
      ...
      <h3 className="text-lg font-semibold leading-snug mb-4" style={{ color: theme.text }}>
        {headline}
      </h3>
      {stats?.length > 0 && (
        <div className="grid grid-cols-2 gap-2.5">
          {stats.slice(0, 4).map((s) => (
            <StatisticCard key={s.label} {...s} theme={theme} size="sm" />
          ))}
        </div>
      )}
    </div>
